-- ============================================================
-- La Bella Infiní — Schema inicial
-- ============================================================
...
-- ============================================================
-- TABELA: blocked_slots (feriados, almoço, férias)
-- ============================================================
...
-- Serviços: leitura pública para serviços ativos
...
-- Promoções: leitura pública para promoções visíveis
...
-- Clientes: admin vê todos, profissional vê apenas os seus
CREATE POLICY "clients_admin_all" ON clients
  FOR ALL USING (
    EXISTS (SELECT 1 FROM staff WHERE user_id = auth.uid())
  );

-- Agendamentos: profissional vê apenas os seus; admin vê todos
...
-- Agendamento anônimo pelo site público
...
INSERT INTO services (name, slug, category, price, duration_minutes, is_active, is_featured, sort_order) VALUES
  ...
  ('Hidratação Facial Intensiva',       'hidratacao-facial',       'facial',      160.00, 60,  TRUE, FALSE, 3),
  ('Extensão de Cílios Fio a Fio',     'extensao-cilios',         'cilios',      250.00, 150, TRUE, TRUE,  4),
  ('Volume Russo',                      'volume-russo',            'cilios',      320.00, 180, TRUE, FALSE, 5),
  ('Manutenção de Cílios',             'manutencao-cilios',       'cilios',      130.00, 90,  TRUE, FALSE, 6),
  ...
  ('Drenagem Linfática',               'drenagem-linfatica',      'corpo',       150.00, 60,  TRUE, TRUE,  9);
...
INSERT INTO promotions (title, discount_type, discount_value, service_ids, starts_at, ends_at, segment, coupon_code, is_visible) VALUES
  ('Boas-vindas — 10% off', 'percent', 10, '{}', NOW(), NOW() + INTERVAL '1 year', 'all', 'BEMVINDA', TRUE);
